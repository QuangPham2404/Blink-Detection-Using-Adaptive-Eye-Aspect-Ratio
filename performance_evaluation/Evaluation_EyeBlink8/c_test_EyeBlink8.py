...
file_path = r"C:\Users\STVN\Desktop\PYTHON\Thuc_tap_BK\blink_notification\performace_assessment\Evaluation_EyeBlink8\ab_calibratied_RWB_EARs.txt"
                ...
                if results.multi_face_landmarks:
                    ...
                    if frame_count_started and smoothen_value >= EAR_THRESH:
                        ...
                        if frame_count <= FRAME_THRESH:
                            ...
                            updated_EARThresh = round((highest_in_blink_sequence+(lowest_in_blink_sequence)*2)/3, 4)
...
'''for index, video_path in enumerate(EAR_paths):
    ...
    blink_count, blink_sequence_frame_indexes_list = test_EyeBlink8(video_path, inital_EARThresh)
    with open(f"result_RWB_{index+1}.txt", "w") as file:
        file.write(str(blink_count) + "\n")
        for lst in blink_sequence_frame_indexes_list:
            file.write(str(lst) + "\n")'''

video_path = r"C:\Users\STVN\Desktop\PYTHON\Thuc_tap_BK\blink_notification\performace_assessment\Evaluation_RealWorldBlink\RealWorldBlink_mp4\quang_output_video1.mp4"
initial_EARThresh = 0.4545
blink_count, blink_sequence_frame_indexes_list = test_EyeBlink8(video_path, initial_EARThresh)
with open(f"result_RWB_{3}.txt", "w") as file:
    file.write(str(blink_count) + "\n")
    for lst in blink_sequence_frame_indexes_list:
        file.write(str(lst) + "\n")