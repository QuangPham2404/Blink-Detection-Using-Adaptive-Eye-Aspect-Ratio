...
with open(r"C:\Users\STVN\Desktop\PYTHON\Thuc_tap_BK\blink_notification\new_svm_extended_dataset\b_svm_exdataset_database.txt", "r") as file:
    database = []
    for line in file:
...
with open(r"C:\Users\STVN\Desktop\PYTHON\Thuc_tap_BK\blink_notification\new_svm_extended_dataset\b_svm_exdataset_target.txt", "r") as file:
    target = []
    for line in file:
        try:
...
joblib.dump(clf, 'ear_svm_exdataset_model.pkl')

# Load the model from the file (for later use)
clf = joblib.load('ear_svm_exdataset_model.pkl')